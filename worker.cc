...
static const std::string kWorkerType = "<YOUR PROJECT NAME HERE>";
static const std::string kLoggerName = "worker.cc";

int main(int argc, char** argv) {
  if (argc != 4) {
    std::cout << "Usage: " << argv[0] << " <hostname> <port> <worker id>" << std::endl;
    std::cout << "For example: " << argv[0] << " localhost 7777 Worker0" << std::endl;
    return 1;
  }
